...
function Capture() 
{
   ...
   var autoFlag = 1;
    ...
    else
    {
        ...
        {    
            // BMap_mask
            var X = $('.BMap_mask').offset().top;
            var  Y= $('.BMap_mask').offset().left;
             return captureObj.DoCapture("1.jpg", hideFlag, 1, Y, X+document.body.offsetHeight,$('.BMap_mask').width(),$('.BMap_mask').height());
        }
        else
        {   
            // var X = $('#chart').parent().parent().offset().top;
            // var  Y= $('#chart').parent().parent().offset().left;
            var div1 = document.getElementById("chart").parentNode.parentNode;
            // alert("距顶:"+div1.X+",距左:"+div1.Y);
            return captureObj.DoCapture("1.jpg", hideFlag, 1,div1.offsetLeft,document.body.offsetHeight+div1.offsetTop,1000,600);
        }
    }            
}
...
function UploadData()
{
    ...
    var data =savedPictureContent;	
             
    $.ajax({
            type: "POST",
            url: "index.php/Data/screenshot/",
            dataType: "text",
            data:{"img":data,"shuxin":a}, 
            ...
            error : function(){ //layer.alert('由于网络原因，上传失败。');
         }
        });  
}  
